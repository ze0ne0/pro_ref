...
CacheMasterCntlr::~CacheMasterCntlr()
{
   delete m_cache;
		...
		if(prak_stat)
		{	PRAK_LOG("CALLING STAT DESTRUCTOR");
			delete prak_stat; 
			prak_stat=NULL;
		}
   ...
   m_shmem_perf_model(shmem_perf_model)
{
   ...
   if (isMasterCache())
   {
	...
	if(MemComponent::L2_CACHE==mem_component)
	{
		m_master->prak_stat=new PrakStats();
	}
	else
	{
...
//	PRAK_LOG("CACHE:%s L2_access:%lld L2_hits:%lld",MemComponentString(m_mem_component),L2_accesses,L2_hits);
      delete m_master;
   }
   delete m_shmem_perf;
...
CacheCntlr::processShmemReqFromPrevCache(CacheCntlr* requester, Core::mem_op_t mem_op_type, IntPtr address, bool modeled, bool count, Prefetch::prefetch_type_t isPrefetch, SubsecondTime t_issue, bool have_write_lock)
{
   ...
   bool cache_hit = operationPermissibleinCache(address, mem_op_type,NULL,true), sibling_hit = false, prefetch_hit = false;
